...
unsigned char *fb_open(char *fbname, struct framebuffer *fbp)
{
	...
	//void *mmap(void *addr, size_t length, int prot, int flags,
        //          int fd, off_t offset);
	fbp->fb_starting = mmap(NULL, fbp->fb_screensize, PROT_READ 
				| PROT_WRITE, MAP_SHARED, fbp->fb_fd, 0);
	if((void *)fbp->fb_starting == MAP_FAILED){
	...
	fprintf(stdout, "fb: %dx%d/%dbits, screen: %lu btyes, "
		...
		(unsigned int)fbp->fb_starting);
#endif

	return fbp->fb_starting;
}
...
int fb_close(struct framebuffer *fbp)
{
	...
	if(munmap(fbp->fb_starting, fbp->fb_screensize) < 0){
